...
#if defined(WIN32)
#pragma comment(lib, "./ZeusingAPI/zeusingtraderapi.lib")
#else
...
bool IsErrorRspInfo(CZeusingFtdcRspInfoField *pRspInfo)
{
...
void PrintRtnOrder(CZeusingFtdcOrderField* pOrder)
{
	ostringstream oss;
	oss << "\n///////////////////////\n";
...
boost::tuple<bool, string> CTradeAgent::Login(const string& frontAddr, const string& brokerId, const string& investorId, const string& userId, const string& password)
{
	try{
		...
		// 创建UserApi
		m_pUserApi = CZeusingFtdcTraderApi::CreateFtdcTraderApi(streamFolder.c_str());
		// 注册事件类
		m_pUserApi->RegisterSpi(this);

		m_pUserApi->SubscribePrivateTopic(ZEUSING_TERT_QUICK);
		m_pUserApi->SubscribePublicTopic(ZEUSING_TERT_QUICK);
...
void CTradeAgent::OnRspUserLogin( CZeusingFtdcRspUserLoginField *pRspUserLogin, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
void CTradeAgent::OnRspSettlementInfoConfirm( CZeusingFtdcSettlementInfoConfirmField *pSettlementInfoConfirm, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	if (bIsLast)
	{
...
void CTradeAgent::OnRspUserLogout( CZeusingFtdcUserLogoutField *pUserLogout, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
{
	CZeusingFtdcInputOrderField req;
	memset(&req, 0, sizeof(req));
	///经纪公司代码
...
bool CTradeAgent::SubmitOrder( CZeusingFtdcInputOrderField& inputOrderField )
{
	int iRequestID = RequestIDIncrement();
...
void CTradeAgent::OnRspOrderInsert( CZeusingFtdcInputOrderField *pInputOrder, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	ostringstream oss;
...
void CTradeAgent::OnRtnOrder( CZeusingFtdcOrderField *pOrder )
{
	if(!IsMyOrder(pOrder))
		return;
	...
	if(pOrder->OrderSubmitStatus >= ZEUSING_FTDC_OSS_InsertRejected)
...
void CTradeAgent::OnRtnTrade( CZeusingFtdcTradeField *pTrade )
{
	ostringstream oss;
...
{
	CZeusingFtdcInputOrderActionField req;
	memset(&req, 0, sizeof(req));
	///经纪公司代码
	strcpy_s(req.BrokerID, m_brokerId.c_str());
	///投资者代码
...
void CTradeAgent::OnRspOrderAction( CZeusingFtdcInputOrderActionField *pInputOrderAction, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
void CTradeAgent::OnRspQryTradingAccount( CZeusingFtdcTradingAccountField *pTradingAccount, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
{
	CZeusingFtdcQryInvestorPositionDetailField req;
	memset(&req, 0, sizeof(req));
	strcpy_s(req.BrokerID, m_brokerId.c_str());
	strcpy_s(req.InvestorID, m_investorId.c_str());
...
void CTradeAgent::OnRspQryInvestorPositionDetail( CZeusingFtdcInvestorPositionDetailField *pInvestorPositionDetail, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	if(!IsErrorRspInfo(pRspInfo))
	{
...
{
	CZeusingFtdcQryDepthMarketDataField req;
	strcpy_s(req.InstrumentID, symbol.c_str());
...
void CTradeAgent::OnRspQryDepthMarketData( CZeusingFtdcDepthMarketDataField *pDepthMarketData, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
{
	CZeusingFtdcQryInstrumentField req;
	memset(&req, 0, sizeof(req));
...
void CTradeAgent::OnRspQryInstrument( CZeusingFtdcInstrumentField *pInstrument, CZeusingFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	if(pInstrument != NULL)
	{
